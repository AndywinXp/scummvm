...
IMuseDigiTriggersHandler::IMuseDigiTriggersHandler(Imuse *engine) {
	_engine = engine;
	_emptyMarker[0] = '\0';
}
...
void IMuseDigiTriggersHandler::saveLoad(Common::Serializer &ser) {
	for (int l = 0; l < DIMUSE_MAX_TRIGGERS; l++) {
		...
		ser.syncAsSint32LE(_trigs[l].transitionType, VER(103));
	...
	for (int l = 0; l < DIMUSE_MAX_DEFERS; l++) {
		...
		ser.syncAsSint32LE(_defers[l].transitionType, VER(103));
...
int IMuseDigiTriggersHandler::setTrigger(int soundId, char *marker, int transitionType, int d, int e, int f, int g, int h, int i, int j, int k, int l, int m, int n) {
	if (soundId == 0) {
		return -5;
	}
	...
	if (strlen(marker) >= 256) {
		Debug::debug(Debug::Sound, "IMuseDigiTriggersHandler::setTrigger(): ERROR: attempting to set trigger with oversized marker string");
		return -5;
	}
	...
	for (int index = 0; index < DIMUSE_MAX_TRIGGERS; index++) {
		if (_trigs[index].sound == 0) {
			...
			_trigs[index].transitionType = transitionType;
			...
			Debug::debug(Debug::Sound, "IMuseDigiTriggersHandler::setTrigger(): Successfully set trigger for soundId %d and marker '%s'", soundId, marker);
			return 0;
		}
	}
	Debug::debug(Debug::Sound, "IMuseDigiTriggersHandler::setTrigger(): ERROR: unable to allocate trigger \"%s\" for sound %d, every slot is full", marker, soundId);
	return -6;
}

int IMuseDigiTriggersHandler::checkTrigger(int soundId, char *marker, int transitionType) {
	int r = 0;
	for (int l = 0; l < DIMUSE_MAX_TRIGGERS; l++) {
		if (_trigs[l].sound != 0) {
			if (soundId == -1 || _trigs[l].sound == soundId) {
				if (!strcmp(marker, _emptyMarker) || !strcmp(marker, _trigs[l].text)) {
					if (transitionType == -1 || _trigs[l].transitionType == transitionType)
						r++;
				}
			}
		}
...
int IMuseDigiTriggersHandler::clearTrigger(int soundId, char *marker, int transitionType) {
	for (int l = 0; l < DIMUSE_MAX_TRIGGERS; l++) {
		if ((_trigs[l].sound != 0) && (soundId == -1 || _trigs[l].sound == soundId) &&
            (!strcmp(marker, _emptyMarker) || !strcmp(marker, _trigs[l].text)) &&
            (transitionType == -1 || _trigs[l].transitionType == transitionType)) {
...
void IMuseDigiTriggersHandler::processTriggers(int soundId, char *marker) {
	...
	int r;
	if (strlen(marker) >= 256) {
		Debug::debug(Debug::Sound, "IMuseDigiTriggersHandler::processTriggers(): ERROR: the input marker string is oversized");
		return;
	}
	...
	for (int l = 0; l < DIMUSE_MAX_TRIGGERS; l++) {
		...
		Debug::debug(Debug::Sound, "IMuseDigiTriggersHandler::processTriggers(): executing trigger for soundId %d and marker '%s'", soundId, marker);
		if (_trigs[l].transitionType == 0) {
			// Call the script callback (a function which sets _stoppingSequence to 1)
			_engine->scriptTriggerCallback(_textBuffer);
		} else {
			if (_trigs[l].transitionType < 30) {
				// Execute a command
				_engine->cmdsHandleCmd(_trigs[l].transitionType, nullptr,
...
int IMuseDigiTriggersHandler::deferCommand(int count, int transitionType, int c, int d, int e, int f, int g, int h, int i, int j, int k, int l, int m, int n) {
	if (!count) {
		return -5;
	}
	for (int index = 0; index < DIMUSE_MAX_DEFERS; index++) {
		if (!_defers[index].counter) {
			...
			_defers[index].transitionType = transitionType;
			_defers[index].a = c;
			_defers[index].b = d;
			_defers[index].c = e;
			_defers[index].d = f;
	...
	Debug::debug(Debug::Sound, "IMuseDigiTriggersHandler::deferCommand(): ERROR: couldn't allocate deferred command");
	return -6;
}
...
void IMuseDigiTriggersHandler::loop() {
	...
	for (int l = 0; l < DIMUSE_MAX_DEFERS; l++) {
		...
		if (_defers[l].counter == 1) {
			if (_defers[l].transitionType != 0) {
				if (_defers[l].transitionType < 30) {
					_engine->cmdsHandleCmd(_trigs[l].transitionType, nullptr,
						_trigs[l].a, _trigs[l].b,
						_trigs[l].c, _trigs[l].d,
						_trigs[l].e, _trigs[l].f,
...
int IMuseDigiTriggersHandler::countPendingSounds(int soundId) {
	...
	for (int l = 0; l < DIMUSE_MAX_TRIGGERS; l++) {
		...
		int transitionType = _trigs[l].transitionType;
		if ((transitionType == 8 && _trigs[l].a == soundId) || (transitionType == 26 && _trigs[l].b == soundId)) {
			r++;
		}
	}
	...
	for (int l = 0; l < DIMUSE_MAX_DEFERS; l++) {
		...
		int transitionType = _defers[l].transitionType;
		if ((transitionType == 8 && _defers[l].a == soundId) || (transitionType == 26 && _defers[l].b == soundId)) {
			r++;
		}
	}
