...
int DiMUSE_InternalMixer::mixer_loop(uint8 **destBuffer, int len) {
	...
	if (!mixer_stereoReverseFlag || mixer_outChannelCount == 1) {
		if (mixer_outWordSize != 16) {
			if (len) {
				for (int i = 0, j = len - 1; j > 0; i++, j--) {
					destBuffer_tmp[i] = ((uint8 *)mixer_softLMID)[mixBuffer[i]];
				}
			}
		}
		...
		if (len) {
			for (int i = 0; i < len; i++) {
				((uint16 *)destBuffer_tmp)[i] = ((uint16 *)mixer_softLMID)[mixBuffer[i]];
			}
		}
	} else {
		len /= 2;
		if (mixer_outWordSize == 16) {
			if (len) {
				for (int i = 0; i < len; i += 2) {
					((uint16 *)destBuffer_tmp)[i] = ((uint16 *)mixer_softLMID)[mixBuffer[i + 1]];
					((uint16 *)destBuffer_tmp)[i + 1] = ((uint16 *)mixer_softLMID)[mixBuffer[i]];
				}
			}
		}
		...
		if (len) {
			for (int i = 0; i < len; i += 2) {
				destBuffer_tmp[i] = ((uint8 *)mixer_softLMID)[mixBuffer[i + 1]];
				destBuffer_tmp[i + 1] = ((uint8 *)mixer_softLMID)[mixBuffer[i]];
			}
		}
	}
	return 0;
	...
	if (feedSize == inFrameCount) {
		if (inFrameCount / 2) {
			...
			for (int i = 0; i < inFrameCount / 2; i++) {
				...
				mixBufCurCell[1] += ((int16 *)ampTable)[(srcBuf_ptr[2] | (16 * (srcBuf_ptr[1] & 0xF0)))];
				srcBuf_ptr += 3;
				mixBufCurCell += 2;
			}
		}
		...
		if ((inFrameCount / 2) - 1) {
			for (int i = 0; i < (inFrameCount / 2) - 1; i++) {
				value = ((int16 *)ampTable)[(srcBuf_ptr[2] | (16 * (srcBuf_ptr[1] & 0xF0)))];

				mixBufCurCell[0] += ((int16 *)ampTable)[srcBuf_ptr[0] | ((srcBuf_ptr[1] & 0xF) << 8)];
				mixBufCurCell[1] += (value + ((int16 *)ampTable)[value]) >> 1;
				mixBufCurCell[2] += value;
				mixBufCurCell[3] += (value + ((int16 *)ampTable)[(srcBuf_ptr[3] | ((srcBuf_ptr[4] & 0xF) << 8))]) >> 1;
				srcBuf_ptr += 3;
				mixBufCurCell += 4;
			}
		}
		...
		mixBufCurCell[1] += (((int16 *)ampTable)[(srcBuf_ptr[0] | ((srcBuf_ptr[1] & 0xF) << 8))]
			               + ((int16 *)ampTable)[(srcBuf_ptr[2] | (16 * (srcBuf_ptr[1] & 0xF0)))]) >> 1;
		mixBufCurCell[2] +=  ((int16 *)ampTable)[(srcBuf_ptr[2] | (16 * (srcBuf_ptr[1] & 0xF0)))];
		mixBufCurCell[3] +=  ((int16 *)ampTable)[(srcBuf_ptr[2] | (16 * (srcBuf_ptr[1] & 0xF0)))];
	} else if (2 * feedSize == inFrameCount) {
		if (feedSize) {
			srcBuf_ptr = srcBuf;
			for (int i = 0; i < feedSize; i++) {
		...
		if (feedSize) {
			...
			for (int i = 0; i < feedSize; i++) {
				...
				mixBufCurCell[i] += ((int16 *)ampTable)[(term_2 | term_1)];
				residualLength += inFrameCount;
...
void DiMUSE_InternalMixer::mixer_mixBits16Mono(uint8 *srcBuf, int inFrameCount, int feedSize, int mixBufStartIndex, int *ampTable) {
	...
	if (feedSize == inFrameCount) {
		if (feedSize) {
			...
			for (int i = 0; i < feedSize; i++) {
				mixBufCurCell[i] += *(uint16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[i] & 0xFFF7) >> 3) + 4096);
			}
		}
	} else if (2 * inFrameCount == feedSize) {
		srcBuf_ptr = (uint16 *)srcBuf;
		...
		if (inFrameCount - 1 != 0) {
			for (i = 0; i < inFrameCount - 1; i++) {
				mixBufCurCell[0] += *(uint16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[i] & 0xFFF7) >> 3) + 4096);
				mixBufCurCell[1] += (*(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[i] & 0xFFF7) >> 3) + 4096)
							       + *(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[i + 1] & 0xFFF7) >> 3) + 4096)) >> 1;
				mixBufCurCell += 2;
			}
		}
		mixBufCurCell[0] += *(uint16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[i] & 0xFFF7) >> 3) + 4096);
		mixBufCurCell[1] += *(uint16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[i] & 0xFFF7) >> 3) + 4096);
	} else {
		if (2 * feedSize == inFrameCount) {
			if (feedSize) {
				srcBuf_ptr = (uint16 *)srcBuf;
				for (int i = 0; i < feedSize; i++) {
					mixBufCurCell[i] += *(uint16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[0] & 0xFFF7) >> 3) + 4096);
					srcBuf_ptr += 2;
				}
			}
		} else {
	...
	if (feedSize == inFrameCount) {
		if (feedSize) {
			...
			for (int i = 0; i < feedSize; i++) {
				...
				mixBufCurCell[i] += (((int16 *)ampTable)[2 * term_1] + term_2) >> 1;
				srcBuf_ptr += 3;
			}
		}
	} else if (2 * inFrameCount == feedSize) {
		...
		if (inFrameCount - 1 != 0) {
			for (int i = 0; i < inFrameCount - 1; i++) {
				...
				mixBufCurCell[1] += (((term_1 + ((int16 *)ampTable)[(srcBuf_ptr[3] | ((srcBuf_ptr[4] & 0xF) << 8))]) >> 1)
					+ ((term_2 + ((int16 *)ampTable)[(srcBuf_ptr[5] | ((srcBuf_ptr[4] & 0xF0) << 4))]) >> 1)) >> 1; // TODO CHECK
...
void DiMUSE_InternalMixer::mixer_mixBits16ConvertToMono(uint8 *srcBuf, int inFrameCount, int feedSize, int mixBufStartIndex, int *ampTable) {
	...
	if (feedSize == inFrameCount) {
		if (feedSize) {
			srcBuf_ptr = (uint16 *)srcBuf;
			for (int i = 0; i < feedSize; i++) {
				mixBufCurCell[i] += (*(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[0] & 0xFFF7) >> 3) + 4096)
								   + *(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[1] & 0xFFF7) >> 3) + 4096)) >> 1;
				srcBuf_ptr += 2;
			}
		}
	} else if (2 * inFrameCount == feedSize) {
		...
		if (inFrameCount - 1 != 0) {
			for (int i = 0; i < inFrameCount - 1; i++) {
				mixBufCurCell[0] += (*(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[1] & 0xFFF7) >> 3) + 4096)
								   + *(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[0] & 0xFFF7) >> 3) + 4096)) >> 1;

				mixBufCurCell[1] +=  (((*(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[0] & 0xFFF7) >> 3) + 4096)
									+   *(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[2] & 0xFFF7) >> 3) + 4096)) >> 1)
									+ ((*(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[1] & 0xFFF7) >> 3) + 4096)
									+   *(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[3] & 0xFFF7) >> 3) + 4096)) >> 1)) >> 1;
				mixBufCurCell += 2;
				srcBuf_ptr += 2;
			}
		}
		...
		mixBufCurCell[0] += (*(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[1] & 0xFFF7) >> 3) + 4096)
						   + *(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[0] & 0xFFF7) >> 3) + 4096)) >> 1;
		mixBufCurCell[1] += (*(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[1] & 0xFFF7) >> 3) + 4096)
						   + *(int16 *)((uint8 *)ampTable + ((int16)(srcBuf_ptr[0] & 0xFFF7) >> 3) + 4096)) >> 1;
	} else if (2 * feedSize == inFrameCount) {
		if (feedSize) {
			srcBuf_ptr = (uint16 *)srcBuf;
...
void DiMUSE_InternalMixer::mixer_mixBits12ConvertToStereo(uint8 *srcBuf, int inFrameCount, int feedSize, int mixBufStartIndex, int *leftAmpTable, int *rightAmpTable) {
	...
	if (feedSize == inFrameCount) {
		if (inFrameCount / 2) {
			srcBuf_ptr = srcBuf;
			for (int i = 0; i < (inFrameCount / 2); i++) {
				mixBufCurCell[0] += ((int16 *)leftAmpTable)[(srcBuf_ptr[0] | ((srcBuf_ptr[1] & 0xF) << 8))];
				mixBufCurCell[1] += ((int16 *)rightAmpTable)[(srcBuf_ptr[0] | ((srcBuf_ptr[1] & 0xF) << 8))];
		...
		if ((inFrameCount / 2) - 1 != 0) {
			for (int i = 0; i < (inFrameCount / 2) - 1; i++) {
				...
				mixBufCurCell[4] += ((int16 *)leftAmpTable)[(srcBuf_ptr[2] | ((srcBuf_ptr[1] & 0xF0) << 4))];
				mixBufCurCell[5] += ((int16 *)rightAmpTable)[(srcBuf_ptr[2] | ((srcBuf_ptr[1] & 0xF0) << 4))];
		...
		if (feedSize) {
			srcBuf_ptr = srcBuf;
			for (int i = 0; i < feedSize; i++) {
				mixBufCurCell[0] += ((int16 *)leftAmpTable)[2 * (srcBuf_ptr[0] | ((srcBuf_ptr[1] & 0xF) << 8))];
				mixBufCurCell[1] += ((int16 *)rightAmpTable)[2 * (srcBuf_ptr[0] | ((srcBuf_ptr[1] & 0xF) << 8))];
				srcBuf_ptr += 3;
				mixBufCurCell += 2;
			}
		}
		...
		if (feedSize) {
			...
			for (int i = 0; i < feedSize; i++) {
				...
				mixBufCurCell[0] += ((int16 *)leftAmpTable)[(term_1 | term_2)];
				...
				mixBufCurCell[1] += ((int16 *)rightAmpTable)[(term_3 | term_4)];
...
void DiMUSE_InternalMixer::mixer_mixBits16ConvertToStereo(uint8 *srcBuf, int inFrameCount, int feedSize, int mixBufStartIndex, int *leftAmpTable, int *rightAmpTable) {
	...
	if (feedSize == inFrameCount) {
		if (feedSize) {
			...
			for (int i = 0; i < feedSize; i++) {
				mixBufCurCell[0] += *(uint16 *)((uint8 *)leftAmpTable  + ((int16)(srcBuf_tmp[i] & 0xFFF7) >> 3) + 4096);
				mixBufCurCell[1] += *(uint16 *)((uint8 *)rightAmpTable + ((int16)(srcBuf_tmp[i] & 0xFFF7) >> 3) + 4096);
				mixBufCurCell += 2;
			}
		}
	} else if (2 * inFrameCount == feedSize) {
		...
		if (inFrameCount - 1 != 0) {
			for (i = 0; i < inFrameCount - 1; i++) {
				...
				mixBufCurCell[2] += (*(int16 *)((uint8 *)leftAmpTable + ((int16)(srcBuf_tmp[i]     & 0xFFF7) >> 3) + 4096)
								   + *(int16 *)((uint8 *)leftAmpTable + ((int16)(srcBuf_tmp[i + 1] & 0xFFF7) >> 3) + 4096)) >> 1;

				mixBufCurCell[3] += (*(int16 *)((uint8 *)rightAmpTable + ((int16)(srcBuf_tmp[i]     & 0xFFF7) >> 3) + 4096)
								   + *(int16 *)((uint8 *)rightAmpTable + ((int16)(srcBuf_tmp[i + 1] & 0xFFF7) >> 3) + 4096)) >> 1;
				mixBufCurCell += 4;
			}
		}
		mixBufCurCell[0] += *(uint16 *)((uint8 *)leftAmpTable  + ((int16)(srcBuf_tmp[i] & 0xFFF7) >> 3) + 4096);
		mixBufCurCell[1] += *(uint16 *)((uint8 *)rightAmpTable + ((int16)(srcBuf_tmp[i] & 0xFFF7) >> 3) + 4096);
		mixBufCurCell[2] += *(uint16 *)((uint8 *)leftAmpTable  + ((int16)(srcBuf_tmp[i] & 0xFFF7) >> 3) + 4096);
		mixBufCurCell[3] += *(uint16 *)((uint8 *)rightAmpTable + ((int16)(srcBuf_tmp[i] & 0xFFF7) >> 3) + 4096);
	} else if (2 * feedSize == inFrameCount) {
		if (feedSize) {
			srcBuf_tmp = (uint16 *)srcBuf;
			for (int i = 0; i < feedSize; i++) {
				mixBufCurCell[0] += *(uint16 *)((uint8 *)leftAmpTable  + ((int16)(srcBuf_tmp[0] & 0xFFF7) >> 3) + 4096);
				mixBufCurCell[1] += *(uint16 *)((uint8 *)rightAmpTable + ((int16)(srcBuf_tmp[0] & 0xFFF7) >> 3) + 4096);
				srcBuf_tmp += 2;
				mixBufCurCell += 2;
			}
		}
...
void DiMUSE_InternalMixer::mixer_mixBits8Stereo(uint8 *srcBuf, int inFrameCount, int feedSize, int mixBufStartIndex, int *ampTable) {
	...
	if (feedSize == inFrameCount) {
		if (feedSize) {
			srcBuf_ptr = srcBuf;
			for (int i = 0; i < feedSize; i++) {
				mixBufCurCell[0] += ((int16 *)ampTable)[srcBuf_ptr[0]];
				mixBufCurCell[1] += ((int16 *)ampTable)[srcBuf_ptr[1]];
				srcBuf_ptr += 2;
				mixBufCurCell += 2;
			}
		}
		...
		if (inFrameCount - 1 != 0) {
			for (int i = 0; i < inFrameCount - 1; i++) {
				mixBufCurCell[0] += ((int16 *)ampTable)[srcBuf_ptr[0]];
				mixBufCurCell[1] += ((int16 *)ampTable)[srcBuf_ptr[1]];
				mixBufCurCell[2] += (((int16 *)ampTable)[srcBuf_ptr[0]] + ((int16 *)ampTable)[srcBuf_ptr[2]]) / 2;
				mixBufCurCell[3] += (((int16 *)ampTable)[srcBuf_ptr[1]] + ((int16 *)ampTable)[srcBuf_ptr[3]]) / 2;
				mixBufCurCell += 4;
				srcBuf_ptr += 2;
			}
		}
		mixBufCurCell[0] += ((int16 *)ampTable)[srcBuf_ptr[0]];
		mixBufCurCell[1] += ((int16 *)ampTable)[srcBuf_ptr[1]];
		mixBufCurCell[2] += ((int16 *)ampTable)[srcBuf_ptr[0]];
		mixBufCurCell[3] += ((int16 *)ampTable)[srcBuf_ptr[1]];
	} else if (2 * feedSize == inFrameCount) {
		if (feedSize) {
			srcBuf_ptr = srcBuf;
			for (int i = 0; i < feedSize; i++) {
...
void DiMUSE_InternalMixer::mixer_mixBits12Stereo(uint8 *srcBuf, int inFrameCount, int feedSize, int mixBufStartIndex, int *ampTable) {
	...
	if (feedSize == inFrameCount) {
		if (feedSize) {
			...
			for (int i = 0; i < feedSize; i++) {
				int term_0 = (srcBuf_ptr[0] | ((srcBuf_ptr[1] & 0xF) << 8));
				mixBufCurCell[0] += ((int16 *)ampTable)[term_0];
	...
	if (feedSize == inFrameCount) {
		if (feedSize) {
			...
			for (int i = 0; i < feedSize; i++) { 
				...
				int16 term_0 = (int16)((srcBuf_ptr[0] & 0xFFF7) >> 3);
				int16 term_1 = (int16)((srcBuf_ptr[1] & 0xFFF7) >> 3);
				mixBufCurCell[0] += *(uint16 *)((int8 *)ampTable + term_0 + 0x1000);
				mixBufCurCell[1] += *(uint16 *)((int8 *)ampTable + term_1 + 0x1000);
				srcBuf_ptr += 2;
				mixBufCurCell += 2;
			}
		}
		...
		if (inFrameCount - 1 != 0) {
			for (int i = 0; i < inFrameCount - 1; i++) {
				mixBufCurCell[0] += *(uint16 *)((int8 *)ampTable + ((int16)(srcBuf_ptr[0] & 0xFFF7) >> 3) + 4096);
				mixBufCurCell[1] += *(uint16 *)((int8 *)ampTable + ((int16)(srcBuf_ptr[1] & 0xFFF7) >> 3) + 4096);

				mixBufCurCell[2] += (*(int16 *)((int8 *)ampTable + ((int16)(srcBuf_ptr[0] & 0xFFF7) >> 3) + 4096)
								   + *(int16 *)((int8 *)ampTable + ((int16)(srcBuf_ptr[2] & 0xFFF7) >> 3) + 4096)) >> 1;

				mixBufCurCell[4] += (*(int16 *)((int8 *)ampTable + ((int16)((((uint8 *)srcBuf_ptr)[2] | (((uint8 *)srcBuf_ptr)[3] << 8)) & 0xFFF7) >> 3) + 4096)
							       + *(int16 *)((int8 *)ampTable + ((int16)(srcBuf_ptr[3] & 0xFFF7) >> 3) + 4096)) >> 1;
				mixBufCurCell += 4;
				srcBuf_ptr += 2;
			}
		}
		mixBufCurCell[0] += *(int16 *)((int8 *)ampTable + ((int16)(srcBuf_ptr[0] & 0xFFF7) >> 3) + 4096);
		mixBufCurCell[1] += *(int16 *)((int8 *)ampTable + ((int16)(srcBuf_ptr[1] & 0xFFF7) >> 3) + 4096);
		mixBufCurCell[2] += *(int16 *)((int8 *)ampTable + ((int16)(srcBuf_ptr[0] & 0xFFF7) >> 3) + 4096);
		mixBufCurCell[3] += *(int16 *)((int8 *)ampTable + ((int16)(srcBuf_ptr[1] & 0xFFF7) >> 3) + 4096);
	} else {
		if (2 * feedSize == inFrameCount) {
			if (feedSize) {
				srcBuf_ptr = (uint16 *)srcBuf;
